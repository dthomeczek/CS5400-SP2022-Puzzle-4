...
# Assignment: Puzzle Assignment #4
# Due Date: 3/21/2022
import sys
...
def h(state: GameState, path, target):
    value = target-state.score
    if not state.p_alive:
        value += 5
    value += int(len(path) / 2)
    return value

# Best-First Search Algorithm
def best_first(state: GameState, target_score):
    path = []
    pq = PriorityQueue()
    copy = state.copy()
    ...
    while not pq.empty():
        ...
        for move in valid_moves:
            ...
            # Add heuristic with path to queue
            pq.put((h(copy2, new_path, target_score), new_path))
